    ...
    <div className={`space-y-6 ${className}`}>
      {groups.map((g, gi) => (
        <Reveal key={g.label} delay={gi * 0.04}>
          <div className="grid gap-4 border-t border-cream/10 pt-5 sm:grid-cols-[8.5rem_1fr] sm:gap-7">
            <div className="flex items-center gap-2">
              <span className="text-[10px] text-ember">✳</span>
              <span className="font-mono text-[11px] uppercase tracking-wider text-cream-mut">
                {g.label}
              </span>
            </div>
            <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 lg:grid-cols-4">
              {g.items.map((it) => (
                ...
                >
                  <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-cream/[0.06] text-cream transition-colors duration-300 group-hover:bg-ember/[0.15] group-hover:text-ember">
