...
export default {
  ...
  summary:
    'A web app that reads the behaviour data you already collect and hands your team a clear, prioritised plan to improve any page. It tracks your progress and shares in one click.',
  ...
  overview: {
    ...
    body: [
      'Modern analytics tools can record how visitors behave on a website: where they click, how far they scroll, and which parts of a page hold their attention. That behaviour gets captured as “heatmaps.” The problem is that the raw exports are dense, cryptic tables that most teams never open, let alone act on.',
      'HeatMapIQ closes that gap. It takes the heatmap exports a team already collects and turns them into a plain-English report: a ranked list of what is going wrong on each page, the evidence behind every issue, the recommended fix, and the impact you can expect. Teams tick off each fix as it ships, watch a progress bar fill up, and share a read-only version of the report through a single link.',
    ],
    pull: 'It turns a pile of data nobody reads into a short to-do list everybody can follow.',
  },
  ...
  problems: {
    ...
    intro:
      'Website and marketing teams collect behaviour data, but turning it into action is slow and needs specialist skill. Three pain points come up again and again.',
    items: [
    ...
    outro:
      'The result is guesswork: teams redesign pages on instinct, disagree about priorities, and rarely close the loop on whether a change actually helped.',
  },
  ...
  signals: {
    ...
    intro:
      'HeatMapIQ was built to make behaviour data usable by anyone on the team, not just the analysts. It reads the three standard heatmap types and puts them into everyday language.',
    items: [
      { k: 'Clicks', v: 'Including “dead clicks,” where people tap something that isn’t actually clickable. That’s a clear sign the design is misleading them.', icon: 'MousePointerClick' },
    ...
    outro:
      'Every finding is tagged with a severity level, so the highest-impact fixes rise to the top. Teams work the list from the top down, mark each item as fixed, and share the live report, running progress and all, whenever they need to.',
  ...
  steps: {
    ...
    items: [
      ...
      { k: 'Get an instant analysis', v: 'HeatMapIQ reviews the data and produces a ranked report of dead clicks, drop-off points, and attention gaps, each with the evidence behind it and a recommended fix, in plain English.' },
  ...
  features: {
    ...
    items: [
      ...
      { title: 'Dead-click detection', body: 'Automatically flags clicks that land on things that aren’t clickable, a fast way to spot confusing design.', icon: 'MousePointerClick' },
      ...
      { title: 'One-click sharing', body: 'Generate a read-only public link to any report, live progress included, and switch it off again whenever you like.', icon: 'Share2' },
  ...
  payoff: {
    ...
    items: [
      ...
      { k: 'Recover lost visitors', v: 'Surfacing drop-off points and dead clicks helps move key content into view and remove the friction that quietly costs conversions.' },
      ...
      { k: 'Keep everyone aligned', v: 'A shared, always-current report means the whole team, and clients too, see the same priorities and the same progress.' },
      { k: 'Show the results', v: 'A read-only link with a live progress bar makes it easy to show the work done and the improvements delivered.' },
    ],
  },
  ...
  audiences: {
    ...
    title: 'Who it’s for',
    intro:
      'Made for the people responsible for how a website performs, whether or not they have a data background.',
    items: [
  ...
  stack: {
    intro:
      'A full-stack web application, built to run in a browser so the whole team can use it from anywhere, and so it’s easy to extend over time.',
    rows: [
    ...
    highlight: {
      k: 'Design highlight',
      v: 'AI is optional by design. HeatMapIQ produces a full report on its own, and it can layer AI on top for richer wording when a key is provided, so it never depends on a single external service.',
    },
  },
